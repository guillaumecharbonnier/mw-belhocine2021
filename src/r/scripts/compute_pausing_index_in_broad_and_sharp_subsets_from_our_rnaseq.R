...
smi <- 'out/r/extract_control_sharp_subsets_from_our_rnaseq/NM_genes_associated_with_Broad.bed'
NM_Broad <- import(smi)

smi <- 'out/r/extract_control_sharp_subsets_from_our_rnaseq/NM_genes_associated_with_Sharp1.bed'
NM_Sharp1 <- import(smi)

smi <- 'out/r/extract_control_sharp_subsets_from_our_rnaseq/NM_genes_associated_with_Sharp2.bed'
NM_Sharp2 <- import(smi)

smi <- 'out/r/extract_control_sharp_subsets_from_our_rnaseq/NM_genes_associated_with_Sharp3.bed'
...
outdir="out/r/compute_pausing_index_in_broad_and_sharp_subsets_from_our_rnaseq"
dir.create(outdir)
...
# Then doing again with values asked by Salva
genebodies_start <- 500
genebodies_end <- -500
...
p <- ggplot(d, aes(x=subset, y=PI, color=subset))
...
ggsave(filename=file.path(outdir,'pi_genebodies_trim_500b_boxplot.pdf'), p, width=4.5, height=3.5)
ggsave(filename=file.path(outdir,'pi_genebodies_trim_500b_boxplot.png'), p, width=4.5, height=3.5)
